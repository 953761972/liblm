...
import {
    IMchc_FormDescriptions_Field_Nullable_Arr,
    IMchc_Nurse_OutpatientDocument,
    SLocal_History,
    SLocal_State,
    SMchc_Nurse,
    TIdTypeCompatible,
...
function Pregnancies(props: { id?: TIdTypeCompatible; toAdd?: boolean; toCheck?: boolean }) {
    ...
    const { 护士端_保存并审核 } = use_provoke((s) => s.config)
    const [formData, setFormData] = useState<PartialAll<IMchc_Nurse_OutpatientDocument>>({
        pregnancyInfo: { validateDate: formatDate()! },
    })
    ...
    function onValuesChange(changedValues: any, allValues: any) {
        ...
        archivalInformation_onValuesChange(changedValues, allValues, form, (obj) =>
            setRequiredKeys({ ...requiredKeys, ...obj }),
        )
    }
    function onPrint() {
        if (formData?.id) {
        ...
        return form
            ...
            .then(async (v) => {
                ...
                if (submit_data.id) {
                    ...
                    if (ok) {
                        safe_navigate(
                            `/prenatal-visit/pregnancy/nurse-end?id=${remoteData.id}`,
                            props,
                            { id: remoteData.id },
                            true,
                        )
                ...
                <BaseEditPanelFormFC
                    ...
                    renderBtns={() => {
                        return (
                            <>
                                {_id ? (
                                    <OkButton primary size="large" onClick={onPrint}>
                                        打印
                                    </OkButton>
                                ) : null}
                                <OkButton
                                    hidden={护士端_审核禁用保存 || toCheck || (护士端_保存并审核 && toAdd)}
                                    loading={loading}
                                    primary
                                    size="large"
                                    onClick={() => onFinish()}
                                ...
                                <OkButton
                                    hidden={!searchId || !toCheck}
                                    loading={loading}
                                    primary
                                    size="large"
                                    onClick={() => onFinish('1', true)}
                                >
                                    审核
                                </OkButton>
                                ...
                                <OkButton
                                    ...
                                    primary
                                    size="large"
                                    onClick={() => onFinish('0').then(() => form.resetFields())}
                                >
                                    保存并继续添加孕册
                                </OkButton>
